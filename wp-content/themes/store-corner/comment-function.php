<?php 
// code for comment
function store_corner_comment( $comment, $args, $depth ) 
{
    ...
    <div class="col-xs-12 comment-detail">
           ...
           <div class="col-xs-10 comments-text">
				...
				<?php comment_date(); ?>
				<?php endif; ?>
				<?php _e('at','store-corner');?>&nbsp;<?php comment_time('g:i a'); ?></span></h4>
				<?php comment_text(); 
				comment_reply_link(array_merge( $args, array('reply_text' => $leave_reply,'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
				</div>
				<?php if ( $comment->comment_approved == '0' ) : ?>
				<em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'store-corner' ); ?></em>
				<br/>
				<?php endif; ?>
			</div>
<?php
